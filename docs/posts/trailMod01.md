...
エフェクター全体の駆動電圧が+9Vで固定ならばQ2を取り外す必要は無く、4007Aの跡地からマイコンで+5V（HIGH）/0V（LOW）を与えるだけで事足ります。しかしこの方式で駆動電圧が+10Vを超えると、その半分であるバイアス電圧が+5Vに達し、Q2において「ソース電圧<ゲート電圧」が叶わなくなり制御が効きません。従って、いっそのことFETをフォトリレーで丸ごと置き換えた方が賢明と判断しました。

いくつかのレビューブログでも言及されているように、このエフェクターでは駆動電圧を大きくするとディレイ音が元気になり発振しやすくなります。ただしディレイ用チップたちの受けられる電圧が-18Vまで（※これらのICは負電源で動かす）なので、エフェクターとしての実質的な耐圧は+18V弱になると思われます。
...
序盤で説明した動作①~④をマイコンで実装していきます。今しがた挙げた挿げ替え後の回路を含めた実態配線図を示します。回路図は無いです。

マイコンは大量に在庫しているという理由でPIC12F675を使っています。ソースコードは以下の通りです。
...
ここに至るまでポップノイズと格闘する時間が大半でした。結果的に、少なくとも自分の耳には聞こえないレベルになったので良かったです。
